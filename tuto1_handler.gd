...
func _ready() -> void:
	...
	MusicHandler.play_music(preload("res://Fantasy Vol4 Music Pack/Fantasy Vol4 Hopeful Main.wav"))
	BusSignal.gem_collected.connect(on_gem_collected)
	BusSignal.player_died.connect(on_player_died)
	var key_number_link := "[img]res://Keyboard/%d.png[/img]"
	var key_string_link := "[img]res://Keyboard/%s.png[/img]"
	var dict := {
	...
	label.add_image(preload("res://16 bit gem/green circle gem.png"))
	label.append_text(".")
...
func on_gem_collected() -> void:
	get_tree().change_scene_to_file.call_deferred("res://tuto2.tscn")
